...
=end
            @builder[:mysql][:account].update(
              account['id'],
              {
                "yahoo_auctions_#{type}_cookies_set_datetime": Time.now.strftime('%Y-%m-%d %H:%M:%S'),
              }
            )
...
pp @controller[:yahoo][:api][:auctions].category_tree(0)
    ...
    def manage_bids
      ...
      .each do |account|
        ...
        .my_bid_list
        .each do |auction_id, content|

          my_bid_list.push(auction_id)
          register_bids(
            @controller[:yahoo][:api][:auctions]
        ...
        .each do |record|
          ...
          case record['bids_state_id']
          when @configure[:builder][:mysql][:bids]
               .get_state['bidding']

            if !my_bid_list.include?(
                 record['bids_auction_id']
               ) and
               !my_won_list.include?(
                 record['bids_auction_id']
               )
              ...
              @builder[:mysql][:bids][:self].update(
                record['bids_id'],
                  {
                    state_id: @configure[:builder][:mysql][:bids]
                              .get_state['end']
                  }
              )
            end
        ...
        .each do |param|
          ...
          begin
            ...
            search(param).each do |k,v|
              ...
              if account['yahoo_auctions_buyer_excluded_accounts']
                 .to_s
                 .split(',')
                 .include?(v[:seller_id])
...
begin
  ...
  if e.message.match(/^指定したオークションIDは無効です。$/)
...
pp auction_item[:status]
    ...
    def cache_bids(item)
      ...
      {
        auction_id: item[:auction_id].to_s,
        seller_id: item[:seller_id].to_s,
        title: item[:title].to_s,
        rating_point: item[:rating_point].to_i,
        rating_total_good_rating: item[:rating_total_good_rating].to_i,
        rating_total_normal_rating: item[:rating_total_normal_rating].to_i,
        rating_total_bad_rating: item[:rating_total_bad_rating].to_i,
        auction_item_url: item[:auction_item_url].to_s,
        img_image1: item[:img_image1].to_s,
        img_image2: item[:img_image2].to_s,
        img_image3: item[:img_image3].to_s,
        initprice: item[:initprice].to_i,
        price: item[:price].to_i,
        bidorbuy: item[:bidorbuy].to_i,
        bids: item[:bids].to_i,
        taxin_price: item[:taxin_price].to_i,
        taxin_bidorbuy: item[:taxin_bidorbuy].to_i,
        option_store_icon_url: item[:option_store_icon_url].to_s,
        option_check_icon_url: item[:option_check_icon_url].to_s,
        option_new_icon_url: item[:option_new_icon_url].to_s,
        option_escrow_icon_url: item[:option_escrow_icon_url].to_s,
        option_featured_icon_url: item[:option_featured_icon_url].to_s,
        option_free_shipping_icon_url: item[:option_free_shipping_icon_url].to_s,
        option_wrapping_icon_url: item[:option_wrapping_icon_url].to_s,
        option_buynow_icon_url: item[:option_buynow_icon_url].to_s,
        option_easy_payment_icon_url: item[:option_easy_payment_icon_url].to_s,
        option_gift_icon_url: item[:option_gift_icon_url].to_s,
        option_bundle_icon_url: item[:option_bundle_icon_url].to_s,
        option_item_status_new_icon_url: item[:option_item_status_new_icon_url].to_s,
        option_y_bank_icon_url: item[:option_y_bank_icon_url].to_s,
        option_english_icon_url: item[:option_english_icon_url].to_s,
        option_star_club_icon_url: item[:option_star_club_icon_url].to_s,
        option_charity_icon_url: item[:option_charity_icon_url].to_s,
      }
    end

    def register_bids(item)

      container = cache_bids(item)
      ...
      if id
        ...
        @builder[:mysql][:bids][:self].update(id, container)
      else

        @builder[:mysql][:bids][:self].insert(container)
      end
    end
...
=begin
      initialize_mechanize
      @controller[:yahoo][:auctions][:mechanize].search(param)
=end
    ...
    def vacation_is_over(item)
      ...
      if @configure[:system][:user]
         ...
         item['item_yahoo_auctions_item_id'].to_s.size > 0
...
pp item
    ...
    def place_bids(item)
      ...
      @driver[:web][:selenium].create_driver(:chrome)
      if @controller[:yahoo][:auctions][:self].place_bids(
           @driver[:web][:selenium],
           @logger,
           item
         )
...
pp e.class, e.message, e.backtrace, e.capture_url
    ...
    def place_value_comment(item)
      ...
      ### なんらかの原因で失敗しても、コメント完了フラグは立てる（2019-08-19）
      @builder[:mysql][:item][:self].update(
        item['item_id'],
        {
          commented: 1
