<?php
class SettingItemAccessoriesController extends BasicController
{
  const _INDEX = 'accessories';
  const _LIST  = 'list';
  const _GET   = 'get';
  const _POST  = 'post';
  ...
  private function verify($data)
  {
    $errors = array();
     ...
     if (!strlen($data['name'])) {
      $errors[] = '付属品名を入力してください';
    }

    if (!strlen($data['accessories_ja'])) {
      $errors[] = '付属品の日本語を入力してください';
    }

    if (!strlen($data['accessories_en'])) {
      $errors[] = '付属品の英語を入力してください';
    }
