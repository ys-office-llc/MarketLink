<?php
class SettingItemDescriptionController extends BasicController
{
  const _INDEX = 'description';
  const _LIST  = 'list';
  const _GET   = 'get';
  const _POST  = 'post';
  ...
  private function verify($data)
  {
    $errors = array();
     ...
     if (!strlen($data['name'])) {
      $errors[] = '説明文名を入力してください';
    }

    if (!strlen($data['description_ja_01'])) {
      $errors[] = '説明文[1]の日本語を入力してください';
    }

    if (!strlen($data['description_en_01'])) {
      $errors[] = '説明文[1]の英語を入力してください';
    }
