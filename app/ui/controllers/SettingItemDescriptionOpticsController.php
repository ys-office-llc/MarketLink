<?php
class SettingItemDescriptionOpticsController extends BasicController
{
  const _INDEX = 'description_optics';
  const _LIST  = 'list';
  const _GET   = 'get';
  const _POST  = 'post';
  ...
  private function verify($data)
  {
    $errors = array();
     ...
     if (!strlen($data['name'])) {
      ...
      $errors[] = '光学系名を入力してください';
    }

    if (!strlen($data['description_optics_ja'])) {

      $errors[] = '光学系の日本語を入力してください';
    }

    if (!strlen($data['description_optics_en'])) {

      $errors[] = '光学系の英語を入力してください';
    }
